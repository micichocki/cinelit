...
class UserFilmStat(models.Model):
    user = models.ForeignKey(User, models.CASCADE)
    film = models.ForeignKey(Film, models.CASCADE)
    total_watching_time = models.IntegerField(blank=True, null=True,
                                              help_text="The number of minutes the user has watched totally")
    ...
    def update_stats(self):
        total_time = \
            WatchingSession.objects.filter(user=self.user, film=self.film).aggregate(total_time=Sum('watching_time'))[
                'total_time']
